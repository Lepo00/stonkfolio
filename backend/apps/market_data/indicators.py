...
def calculate_sma(closes: pd.Series, window: int, *, intraday: bool = False) -> list[dict]:
    ...
    return [{"time": _format_time(idx, intraday), "value": round(float(val), 4)} for idx, val in sma.items()]
...
def calculate_rsi(closes: pd.Series, window: int = 14, *, intraday: bool = False) -> list[dict]:
    ...
    return [{"time": _format_time(idx, intraday), "value": round(float(val), 2)} for idx, val in rsi.items()]
